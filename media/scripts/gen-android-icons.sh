...
# ── Zoomed SVG for round icon ────────────────────────────────────────────────
# The key graphic occupies ~x:165–790, y:107–889 in the 1024×1024 canvas.
# For the round variant we render with a tighter viewBox so the key fills
# ~80 % of the icon diameter.  Slight overflow at the very top/bottom is
# invisible because the circular mask clips those corners.
ROUND_SVG=$(mktemp /tmp/android-icon-round-XXXXXX.svg)
trap 'rm -f "$ROUND_SVG"' EXIT
sed 's/viewBox="0 0 1024 1024"/viewBox="86 80 781 781"/' "$SVG" > "$ROUND_SVG"

# ── Render function ───────────────────────────────────────────────────────────
# render <size> <output_path> [svg_file]
render() {
  local size="$1"
  local out="$2"
  local svg="${3:-$SVG}"
...
for entry in "${DENSITIES[@]}"; do
  ...
  render "$size" "$dir/ic_launcher_round.png" "$ROUND_SVG"
