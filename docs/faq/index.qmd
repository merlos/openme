---
title: "FAQ"
description-meta: "Frequently asked questions about openme — how SPA works, differences from fwknop, supported distros, troubleshooting common issues."
keywords: [openme FAQ, single packet authentication questions, fwknop alternative, openme troubleshooting, SPA explained, openme Linux]
---

## Comparisons

Not sure how openme stacks up against the tools you already know? These pages
give a detailed side-by-side breakdown:

- [openme vs fwknop](vs-fwknop.qmd) — the other SPA tool; crypto stack,
  dependencies, mobile support, QR provisioning.
- [openme vs WireGuard](vs-wireguard.qmd) — SPA gate vs VPN tunnel; why they
  complement each other and how to use both.
- [openme vs Tailscale](vs-tailscale.qmd) — mesh VPN with a control plane vs
  zero-cloud SPA; privacy, cloud dependency, and combined setups.
- [openme vs Traditional VPNs](vs-vpn.qmd) — OpenVPN, IPsec/IKEv2; attack
  surface reduction, when each tool is the right choice.
...
Any Linux with nftables (kernel ≥ 3.13, nft ≥ 0.9) or iptables. You just need to download the binary or compile it with Go. It has been tested on Debian and Arch Linux.
...
The TCP health port is never permanently open — it is added as a firewall rule
alongside your other ports only after a valid knock, and removed after
`knock_timeout`. This is intentional: an always-open health port would make
the server visible to scanners.
...
Yes — `post_knock` is optional. `openme knock` sends the knock and exits.
