...
description-meta: "openme threat model — what attacks SPA protects against (port scanning, replay, spoofing) and known limitations (DoS, compromised keys)."
keywords-meta: [openme security model, SPA threat model, firewall stealth, replay attack prevention, port scan prevention, forward secrecy]
---
...
The knock packet is fully encrypted (ChaCha20-Poly1305 AEAD). Its 165 bytes
are computationally indistinguishable from random noise to anyone who does not
hold the server's static Curve25519 private key. The source IP, target IP, and client identity are all hidden.
The ports to be opened are never transmitted — they are determined server-side.
